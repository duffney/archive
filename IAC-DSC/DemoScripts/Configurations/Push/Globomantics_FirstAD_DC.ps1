Configuration GlobomanticsFirstADDC {
...
GlobomanticsFirstADDC -ConfigurationData $ConfigData `
    -safemodeAdministratorCred (Get-Credential -UserName '(Password Only)' `
     ...
     -domainCred (Get-Credential -UserName Globomantics\administrator `
      -Message "New Domain Admin Credential") -OutputPath "c:\dsc"

Set-DscLocalConfigurationManager -Path c:\dsc -Verbose -Force
Start-DscConfiguration -ComputerName localhost -wait -force -Verbose -Path c:\dsc